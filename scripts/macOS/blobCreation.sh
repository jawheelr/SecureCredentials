...
# Icon for Thank You dialog
ICON_URL="https://raw.githubusercontent.com/jawheelr/SecureCredentials/main/media/icons/SecureCredentials.icns"

# Install swiftDialog if Missing
if [[ ! -x "$DIALOG" ]]; then

    echo "swiftDialog not installed. Installing latest version..."

    TMP_DIR=$(mktemp -d)
    PKG_FILE="$TMP_DIR/swiftDialog.pkg"

    # Get Latest swiftDialog Release
    LATEST_PKG_URL=$(curl -fsSL \
        https://api.github.com/repos/swiftDialog/swiftDialog/releases/latest \
        | awk -F '"' \
        '/browser_download_url/ && /\.pkg/ { print $4; exit }')

    # Validate URL
    if [[ -z "${LATEST_PKG_URL:-}" ]]; then
        echo "Failed to retrieve swiftDialog download URL."
        exit 1
    fi

    # Download Package
    curl -L "$LATEST_PKG_URL" -o "$PKG_FILE"
...
if [[ $PIN_CHOICE -eq 0 ]]; then
    ...
    ########################################################
    # User closed / escaped PIN Selection dialog
    ########################################################
    cancel_exit
fi
